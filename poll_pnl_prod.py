...
while True:
    # 0. Reconcile Ground-Truth State with Tradier First
    ...
    try:
        ...
        for i in raw_items:
            ...
            is_live = int(i.get('is_live', 0))
            
            if status == 'ACTIVE' and shares not in ['0', 'None', '0.0', '']:
                if env_val in ['PRODUCTION', 'PROD', 'LIVE'] or is_live == 1:
                    items.append(i)
        ...
        for item in items:
            ...
            # Fixed Direction-Aware Option PnL Math
            if direction == 'PUT':
