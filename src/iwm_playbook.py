...
PLAYBOOK_CONFIG = {
    "ticker": "IWM",
    "spot_target_call": 298.92,
    "spot_target_put": 295.94,
...
def evaluate_call_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price >= _get_dynamic_target("IWM", "spot_target_call", PLAYBOOK_CONFIG["spot_target_call"]):
        return True, "IWM_CALL_TRIGGER"
    return False, "OUT_OF_BOUNDS"

def evaluate_put_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price <= _get_dynamic_target("IWM", "spot_target_put", PLAYBOOK_CONFIG["spot_target_put"]):
        return True, "IWM_PUT_TRIGGER"
    return False, "OUT_OF_BOUNDS"
