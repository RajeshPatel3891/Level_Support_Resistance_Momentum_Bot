...
PLAYBOOK_CONFIG = {
    "ticker": "SPY",
    "spot_target_call": 768.2,
    "spot_target_put": 760.56,
...
def evaluate_call_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price >= _get_dynamic_target("SPY", "spot_target_call", PLAYBOOK_CONFIG["spot_target_call"]):
        return True, "SPY_CALL_TRIGGER"
    return False, "OUT_OF_BOUNDS"

def evaluate_put_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price <= _get_dynamic_target("SPY", "spot_target_put", PLAYBOOK_CONFIG["spot_target_put"]):
        return True, "SPY_PUT_TRIGGER"
    return False, "OUT_OF_BOUNDS"
