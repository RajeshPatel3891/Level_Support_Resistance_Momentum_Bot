...
PLAYBOOK_CONFIG = {
    "ticker": "GOOGL",
    "spot_target_call": 342.31,
    "spot_target_put": 338.91,
...
def evaluate_call_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price >= _get_dynamic_target("GOOGL", "spot_target_call", PLAYBOOK_CONFIG["spot_target_call"]):
        return True, "GOOGL_CALL_TRIGGER"
    return False, "OUT_OF_BOUNDS"

def evaluate_put_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price <= _get_dynamic_target("GOOGL", "spot_target_put", PLAYBOOK_CONFIG["spot_target_put"]):
        return True, "GOOGL_PUT_TRIGGER"
    return False, "OUT_OF_BOUNDS"
