...
PLAYBOOK_CONFIG = {
    "ticker": "META",
    "spot_target_call": 544.9,
    "spot_target_put": 539.48,
...
def evaluate_call_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price >= _get_dynamic_target("META", "spot_target_call", PLAYBOOK_CONFIG["spot_target_call"]):
        return True, "META_CALL_TRIGGER"
    return False, "OUT_OF_BOUNDS"

def evaluate_put_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price <= _get_dynamic_target("META", "spot_target_put", PLAYBOOK_CONFIG["spot_target_put"]):
        return True, "META_PUT_TRIGGER"
    return False, "OUT_OF_BOUNDS"
