...
PLAYBOOK_CONFIG = {
    "ticker": "QQQ",
    "spot_target_call": 714.09,
    "spot_target_put": 706.99,
...
def evaluate_call_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price >= _get_dynamic_target("QQQ", "spot_target_call", PLAYBOOK_CONFIG["spot_target_call"]):
        return True, "QQQ_CALL_TRIGGER"
    return False, "OUT_OF_BOUNDS"

def evaluate_put_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price <= _get_dynamic_target("QQQ", "spot_target_put", PLAYBOOK_CONFIG["spot_target_put"]):
        return True, "QQQ_PUT_TRIGGER"
    return False, "OUT_OF_BOUNDS"
