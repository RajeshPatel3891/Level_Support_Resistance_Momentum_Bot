...
PLAYBOOK_CONFIG = {
    "ticker": "MARA",
    "spot_target_call": 10.83,
    "spot_target_put": 10.71,
...
def evaluate_call_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price >= _get_dynamic_target("MARA", "spot_target_call", PLAYBOOK_CONFIG["spot_target_call"]):
        return True, "MARA_CALL_TRIGGER"
    return False, "OUT_OF_BOUNDS"

def evaluate_put_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price <= _get_dynamic_target("MARA", "spot_target_put", PLAYBOOK_CONFIG["spot_target_put"]):
        return True, "MARA_PUT_TRIGGER"
    return False, "OUT_OF_BOUNDS"
