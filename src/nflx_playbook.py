...
PLAYBOOK_CONFIG = {
    "ticker": "NFLX",
    "spot_target_call": 80.8,
    "spot_target_put": 79.98,
...
def evaluate_call_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price >= _get_dynamic_target("NFLX", "spot_target_call", PLAYBOOK_CONFIG["spot_target_call"]):
        return True, "NFLX_CALL_TRIGGER"
    return False, "OUT_OF_BOUNDS"

def evaluate_put_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price <= _get_dynamic_target("NFLX", "spot_target_put", PLAYBOOK_CONFIG["spot_target_put"]):
        return True, "NFLX_PUT_TRIGGER"
    return False, "OUT_OF_BOUNDS"
