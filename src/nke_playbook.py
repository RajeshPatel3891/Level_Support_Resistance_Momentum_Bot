...
PLAYBOOK_CONFIG = {
    "ticker": "NKE",
    "spot_target_call": 40.53,
    "spot_target_put": 40.13,
...
def evaluate_call_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price >= _get_dynamic_target("NKE", "spot_target_call", PLAYBOOK_CONFIG["spot_target_call"]):
        return True, "NKE_CALL_TRIGGER"
    return False, "OUT_OF_BOUNDS"

def evaluate_put_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price <= _get_dynamic_target("NKE", "spot_target_put", PLAYBOOK_CONFIG["spot_target_put"]):
        return True, "NKE_PUT_TRIGGER"
    return False, "OUT_OF_BOUNDS"
