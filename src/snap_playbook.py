...
PLAYBOOK_CONFIG = {
    "ticker": "SNAP",
    "spot_target_call": 5.11,
    "spot_target_put": 5.05,
...
def evaluate_call_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price >= _get_dynamic_target("SNAP", "spot_target_call", PLAYBOOK_CONFIG["spot_target_call"]):
        return True, "SNAP_CALL_TRIGGER"
    return False, "OUT_OF_BOUNDS"

def evaluate_put_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price <= _get_dynamic_target("SNAP", "spot_target_put", PLAYBOOK_CONFIG["spot_target_put"]):
        return True, "SNAP_PUT_TRIGGER"
    return False, "OUT_OF_BOUNDS"
