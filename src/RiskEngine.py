...
def evaluate_orb_vwap_setup(df_1min, orb_minutes=15):
    ...
    """
    # Safe DataFrame presence check avoiding series ambiguity traps
    if df_1min is None or not hasattr(df_1min, "empty") or df_1min.empty:
    ...
    try:
        ...
        vwap_series = pv.cumsum() / np.where(cum_vol == 0, 1.0, cum_vol)
        df['vwap'] = vwap_series
...
def evaluate_vwap_mean_reversion(df_1min, rsi_period=14):
    ...
    """
    # Safe DataFrame presence check avoiding series ambiguity traps
    if df_1min is None or not hasattr(df_1min, "empty") or df_1min.empty or len(df_1min) < 30:
