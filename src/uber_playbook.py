...
PLAYBOOK_CONFIG = {
    "ticker": "UBER",
    "spot_target_call": 79.14,
    "spot_target_put": 78.36,
...
def evaluate_call_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price >= _get_dynamic_target("UBER", "spot_target_call", PLAYBOOK_CONFIG["spot_target_call"]):
        return True, "UBER_CALL_TRIGGER"
    return False, "OUT_OF_BOUNDS"

def evaluate_put_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price <= _get_dynamic_target("UBER", "spot_target_put", PLAYBOOK_CONFIG["spot_target_put"]):
        return True, "UBER_PUT_TRIGGER"
    return False, "OUT_OF_BOUNDS"
