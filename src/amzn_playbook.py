...
PLAYBOOK_CONFIG = {
    "ticker": "AMZN",
    "spot_target_call": 262.69,
    "spot_target_put": 260.07,
...
def evaluate_call_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price >= _get_dynamic_target("AMZN", "spot_target_call", PLAYBOOK_CONFIG["spot_target_call"]):
        return True, "AMZN_CALL_TRIGGER"
    return False, "OUT_OF_BOUNDS"

def evaluate_put_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price <= _get_dynamic_target("AMZN", "spot_target_put", PLAYBOOK_CONFIG["spot_target_put"]):
        return True, "AMZN_PUT_TRIGGER"
    return False, "OUT_OF_BOUNDS"
