...
PLAYBOOK_CONFIG = {
    "ticker": "BAC",
    "spot_target_call": 62.94,
    "spot_target_put": 62.72,
...
def evaluate_call_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price >= _get_dynamic_target("BAC", "spot_target_call", PLAYBOOK_CONFIG["spot_target_call"]):
        return True, "BAC_CALL_TRIGGER"
    return False, "OUT_OF_BOUNDS"

def evaluate_put_entry(spot_price, vwap, proximity_score, velocity):
    if spot_price <= _get_dynamic_target("BAC", "spot_target_put", PLAYBOOK_CONFIG["spot_target_put"]):
        return True, "BAC_PUT_TRIGGER"
    return False, "OUT_OF_BOUNDS"
