cat << 'INNER_EOF' > inspect_proximity.py
import os
import sys
import json
import requests
...
# Path configuration - resolved with correct underscore syntax
...
def get_live_snapshots(tickers, api_key, secret_key):
    """
    Fetches real-time price snapshot directly from Alpaca V2 market data API.
    Prints verbose HTTP response bodies on failures to isolate administrative errors.
    """
    headers = {
    ...
    feeds = [
        {"url": f"https://data.alpaca.markets/v2/stocks/snapshots?symbols={','.join(tickers)}", "name": "Standard SIP Feed"},
        {"url": f"https://data.alpaca.markets/v2/stocks/snapshots?symbols={','.join(tickers)}&feed=iex", "name": "Free Real-Time IEX Feed"},
        {"url": f"https://data.alpaca.markets/v2/stocks/snapshots?symbols={','.join(tickers)}&feed=delayed_sip", "name": "Delayed SIP Feed"}
    ]
    ...
    for feed in feeds:
        try:
            ...
            if response.status_code == 200:
                ...
                if snapshots:
                    log_msg(f"[✓] Success: Retrieved market data using {feed['name']}.")
                    return snapshots
            else:
                # HIGH VISIBILITY DIAGNOSTIC: Print the raw error body from Alpaca
                try:
                ...
                log_msg(f"[─] Feed '{feed['name']}' rejected (HTTP {response.status_code}): {err_msg.strip()}")
        except Exception as e:
            log_msg(f"[─] Exception on '{feed['name']}': {e}")
            
    return None

def inspect_system_proximity():
    print("\n" + "="*80)
    print(" HARM.AI // REAL-TIME HIGH-CONVICTION PROXIMITY DIAGNOSTICS ")
    print("="*80)
...
INNER_EOF
echo "[✓] inspect_proximity.py written cleanly to your active workspace."
