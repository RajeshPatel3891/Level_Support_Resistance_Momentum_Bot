...
restored_fetch_code = '''def fetch_portfolio_state(page=1, selected_date=None, tenant_id='COMPANY_A'):
    ...
    for t in all_trades:
        ...
        # Original Delta & Option Pricing Calculation
        entry = float(t.get('entry_price', t.get('basis', 0)))
        stored_spot = float(t.get('spot_price', t.get('price', entry)))
        last_price = stored_spot
...
if re.search(pattern, content, re.DOTALL):
    ...
    with open('dashboard_server.py', 'w') as f:
        f.write(content)
    print('[✓] Restored historical delta option PnL logic!')
else:
